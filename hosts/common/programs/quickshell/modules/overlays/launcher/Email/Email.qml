...
Item {
    ...
    Connections {
        ...
        function onSelectedMailChanged() {
            ...
            mailController.activeMailBody = activeItem.body_content && activeItem.body_content.trim() !== "" ? activeItem.body_content : "(No plain text variant available.)";

            // Mark email as read on click
            if (!(activeItem.flags || []).map(f => f.toLowerCase()).includes("seen")) {
