...
Rectangle {
    ...
    onVisibleChanged: {
        // FIXED: Wrap in a strict 'if (visible)' guard to prevent the hidden
        // modal from stealing active application focus at startup.
        if (visible) {
            if (toInput.text === "") {
                loadContactsDatabase();
                toInput.forceActiveFocus();
    ...
    function checkAutocompleteSuggestions(currentText) {
        ...
        for (var i = 0; i < contactsList.length; i++) {
            ...
            if (contact.toLowerCase().startsWith(txt)) {
    ...
    Rectangle {
        ...
        Column {
            ...
            // 1. RECIPIENT FIELD
            Rectangle {
                width: parent.width; height: (typeof theme !== 'undefined' ? theme.defaultCardHeight : 140) - 50; color: composeComp.fieldBg
                radius: (typeof theme !== 'undefined') ? theme.defaultCardRadius : 10
                ...
                Item {
                    ...
                    Text {
                        text: composeComp.currentSuggestion !== "" ? composeComp.currentSuggestion : ""
                        font.family: composeComp.composeFontFamily
                        font.pixelSize: composeComp.inputFontSize
                        color: (typeof theme !== 'undefined') ? theme.base0B : "#545454"
                        anchors.fill: parent
                        visible: toInput.text !== "" && composeComp.currentSuggestion.toLowerCase().startsWith(toInput.text.toLowerCase())
                    }
                    ...
                    TextInput {
                        ...
                        onTextChanged: {
                            composeComp.checkAutocompleteSuggestions(text);
                        }

                        Keys.onPressed: (event) => {
                            if (event.key === Qt.Key_Tab) {
                                if (composeComp.currentSuggestion !== "" && text !== composeComp.currentSuggestion) {
            ...
            // 3. BODY MESSAGE CONTENT CANVAS (WITH FLICKABLE SCROLLING CANVAS)
            Rectangle {
                width: parent.width; height: parent.height - 240; color: composeComp.fieldBg
                radius: (typeof theme !== 'undefined') ? theme.defaultCardRadius : 10
