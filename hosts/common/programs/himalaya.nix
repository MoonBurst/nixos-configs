    ...
    try:
        ...
        for row in rows:
            ...
            if action == "DELETE":
                ...
                folder_mapping = {
                    "inbox": "INBOX", "starred": ".[Gmail].Starred", "all": ".[Gmail].All Mail",
                ...
                for idx, item in enumerate(envelopes):
                    if isinstance(item, dict):
                        ...
                        if msg_unique_id in existing_bodies:
                            ...
                            # Reverted to robust plain-text read to prevent any JSON-nesting/clutter bugs on disk
                        ...
                        # Filesystem-first visual attachment preview pre-fetch parser (restored)
    ...
    try:
        ...
        for item in uncached_items:
            ...
            if res.returncode == 0:
                ...
                # Filesystem-first visual attachment preview backfill parser (restored)
