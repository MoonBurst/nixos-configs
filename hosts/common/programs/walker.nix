{ inputs, config, pkgs, lib, ... }: {
  home-manager.users.moonburst = { lib, ... }: {
    ...
    # Using mkForce on everything to override the hidden "on-failure" definition
    ...
    systemd.user.services.walker = {
      Unit = {
        ...
        After = lib.mkForce [ "elephant.service" "sway-session.target" ];
        PartOf = lib.mkForce [ "sway-session.target" ];
      };
      Service = {
      ...
      themes."stylix-match" = {
        ...
        in ''
          * { all: unset; font-family: "${font}"; }
          .box-wrapper { background-color: ${c.base00}; padding: 8px; border-radius: 12px; border: 4px solid ${c.base0E}; min-height: 0px; }
          .input { caret-color: ${c.base05}; background-color: ${c.base01}; padding: 8px; color: ${c.base05}; border-radius: 8px; border: 4px solid ${c.base0E}; margin-bottom: 5px; font-size: 16pt; }
          .list { color: ${c.base05}; border: 4px solid ${c.base0E}; border-radius: 10px; padding: 4px; margin-right: 5px; }
          .preview { border: 4px solid ${c.base0E}; border-radius: 10px; background-color: ${c.base01}; padding: 8px; margin-left: 5px; }
